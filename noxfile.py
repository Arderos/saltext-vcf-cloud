...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/vcf_cloud/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/vcf_cloud/*")
            ...
            #    "--include=src/saltext/vcf_cloud/*,tests/*"
            # )
        finally:
